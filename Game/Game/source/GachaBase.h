...
public:
	...
	// ガチャの結果の行データを取得
	const std::vector<std::string>& GetResultRows() const { return _resultRows; }

protected:
	// 純粋仮想関数：派生クラスで実装する必要がある
	virtual bool LoadRows(std::vector<TRow>& outRows, std::string* outError) = 0;
	virtual std::string MakeResultRow(const TRow& row, std::mt19937& rng) = 0;

private:
	std::vector<TRow> _rows;				// ガチャの行データ
	std::mt19937 _rng;						// ガチャの乱数生成器
	std::vector<std::string> _resultRows;	// ガチャの結果の行データ
